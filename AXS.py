...
ASX Stock Analyzer
==================
Search ASX-listed stocks and view fundamental + technical analysis.

Requirements:
    pip install yfinance pandas numpy requests tabulate colorama ta

Usage:
    python asx_stock_analyzer.py
    python asx_stock_analyzer.py --ticker CBA    # Direct lookup
    python asx_stock_analyzer.py --search bank   # Search by keyword
"""
...
def quant_score(hist: pd.DataFrame) -> dict:
    ...
    # Lower 30-day realized volatility = safer entry (less whipsaw risk)
    # Higher volatility = riskier but potentially higher reward
    daily_ret = close.pct_change().dropna()
    rv_30 = _safe(daily_ret.tail(30).std() * math.sqrt(252) * 100)  # Annualized %
    ...
    # RSI<30 = oversold (potential bounce/reversal BUY signal)
    # RSI>70 = overbought (potential pullback/reversal SELL signal)
    ...
    rsi   = _safe(rsi, 50)
    # RSI <30 → score 90 (oversold, buying opportunity)
    # RSI >70 → score 10 (overbought, selling pressure)
    # RSI =50 → score 50 (neutral momentum)
    ...
    # VWAP = smart money's average entry price over the period
    ...
    # VWAP is directional: rising VWAP in uptrend, falling VWAP in downtrend
...
def game_theory_score(hist: pd.DataFrame, quant: dict, info: dict) -> dict:
    ...
    # Nash Equilibrium: Self-reinforcing state where no player benefits from changing strategy
    # If all 3 players agree on BUY → coordinates bull market (high NE score)
    # If all 3 players agree on SELL → coordinates bear market (low NE score)
    # If players disagree → market conflict, uncertain direction (medium NE score)
    strategies = [inst_signal, retail_signal, mm_signal]
    ne_votes   = {s: strategies.count(s) for s in ("BUY","HOLD","SELL")}  # Vote count
    ne_dominant = max(ne_votes, key=ne_votes.get)                           # Majority strategy
    ne_strength = ne_votes[ne_dominant] / 3 * 100   # Strength: 33% (split), 67% (2-1), 100% (unanimous)

    # Score mapping: Unanimous BUY = 100, Unanimous SELL = 0, Split = ~67
    ne_score = (
        70 + ne_strength * 0.3 if ne_dominant == "BUY"  else  # Max 100 when unanimous BUY
        50                     if ne_dominant == "HOLD" else  # Neutral
        30 - ne_strength * 0.3                              # Min 0 when unanimous SELL
    )
...
def search_stocks(query: str):
    """Search ASX stocks by ticker or company name keyword."""
    query = query.upper().strip()
    results = []
    for ticker, name in ASX_STOCKS.items():
...
def fetch_stock(ticker: str):
    """Fetch yfinance Ticker for an ASX stock (appends .AX if needed)."""
    asx_ticker = ticker.upper()
    if not asx_ticker.endswith(".AX"):
        asx_ticker += ".AX"
    stock = yf.Ticker(asx_ticker)
    return stock, asx_ticker
    ...
    for section, keys in sections.items():
        ...
        for k in keys:
            ...
            if k in ("Market Cap", "Enterprise Value", "Revenue (TTM)", "EBITDA",
                ...
                display = f"A${fmt(v)}" if v is not None else "N/A"
        ...
        for k in keys:
            ...
            if v is None or (isinstance(v, float) and np.isnan(float(v))):
                ...
                if k in ("Price vs SMA20","Price vs SMA50","Price vs SMA200"):
                    ...
                    display = f"{col}{sign}A${fv:.4f}{Style.RESET_ALL}"
...
def ascii_chart(hist, width=55, height=12):
    ...
    print(f"  High: A${mx:.2f}  Low: A${mn:.2f}")
...
def overall_signal(fund, tech):
    ...
    if fund:
        ...
        if pe and not np.isnan(float(pe if pe else np.nan)):
            signals.append(("BUY" if float(pe) < 20 else "SELL", f"P/E={float(pe):.1f}"))
...
def analyse(ticker: str, period: str = "1y"):
    ...
    hist = stock.history(period=period)
    if hist.empty:
        print(f"{Fore.RED}  ✘  No data found for {asx_ticker}. "
              f"Check the ticker or try adding .AX manually.{Style.RESET_ALL}")
        return
    ...
    print(f"\n{Fore.WHITE}  {'─'*58}")
    print(f"  {Fore.YELLOW}{asx_ticker}{Style.RESET_ALL}   "
          f"Last: {Fore.WHITE}A${current_price:.4f}{Style.RESET_ALL}   "
          f"Day: {day_change}   "
          f"Period: {period}")
    ...
    if fund:
        print_fundamentals(fund, asx_ticker)
    else:
    ...
    if tech_summary:
        print_technicals(tech_summary, asx_ticker, current_price)
    else:
...
def interactive_search():
    ...
    while True:
        ...
        print(f"  {Fore.YELLOW}[3]{Style.RESET_ALL} View popular ASX stocks")
        ...
        if choice == "q":
            ...
            ticker = input(f"  Enter ASX ticker (e.g. CBA, BHP, CSL): ").strip().upper()
            if ticker:
                period = input(f"  Period (1mo/3mo/6mo/1y/2y/5y) [default 1y]: ").strip() or "1y"
                analyse(ticker, period)

        elif choice == "3":
            rows = [(t, n) for t, n in list(ASX_STOCKS.items())]
            print(f"\n{Fore.YELLOW}  Popular ASX Stocks:{Style.RESET_ALL}")
            print(tabulate(rows, headers=["Ticker", "Company"], tablefmt="rounded_outline"))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ASX Stock Analyser — Fundamentals & Technicals")
    parser.add_argument("--ticker", "-t", help="ASX ticker symbol (e.g. CBA, BHP)")
    parser.add_argument("--search", "-s", help="Search stocks by keyword")
    parser.add_argument("--scan", action="store_true", help="Run deep scanner on all ASX stocks")
    ...
    if args.dashboard:
        ...
        run_deep_scanner(ASX_STOCKS, fetch_stock, "A$",
